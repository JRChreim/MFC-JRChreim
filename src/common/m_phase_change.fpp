...
module m_phase_change
 ...
 s_infinite_relaxation_k
    ...
    real(wp), parameter :: mixM = 1*sgm_eps               !< threshold for 'mixture cell'. If Y < mixM, phase change does not happen
...
&                   . Please, check energy constraint. Econst ~'//Econsts//'. Aborting!')
    ...
    subroutine s_infinite_ptg_relaxation(j, k, l, alphak, me0k, m0k, pS, p_infpT, rho, rhoe, rM, TR, TS)
        ...
        if ((pS < -2.25e6_wp) .and. (rM > (rhoe - gs_min(lp)*ps_inf(lp)/(gs_min(lp) - 1.0e-1_wp))/qvs(lp))) then
